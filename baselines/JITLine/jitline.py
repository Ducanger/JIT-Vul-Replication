...
def run_experiment(cur_proj, split_data='by_time'):
    data_path = f'data/jitline/{split_data}/'
    model_path = f'model/jitline/{split_data}/'
    ...
    print('executing smote', int(np.round(result.x)))
...
def eval_metrics(result_df):
    ...
    return prec, rec, f1, acc, AUC, recall_20_percent_effort, effort_at_20_percent_LOC_recall, p_opt


def eval_result(proj_name, split_data='by_time', sampling_method='DE_SMOTE_min_df_3'):
    data_path = f'data/jitline/{split_data}/'
    RF_data_dir = f'data/jitline/{split_data}/'
    ...
    prec, rec, f1, acc, auc, recall_20_percent_effort, effort_at_20_percent_LOC_recall, p_opt = eval_metrics(
        RF_result)
    print(
        'Precision: {:.4f}, Recall: {:.4f}, F1: {:.4f},  AUC: {:.4f}, ACC: {:.4f}, PCI@20%LOC: {:.4f}, Effort@20%Recall: {:.4f}, POpt: {:.4f}'.format(
            prec, rec, f1, acc, auc, recall_20_percent_effort, effort_at_20_percent_LOC_recall, p_opt))
...
if __name__ == '__main__':
    ...
    k_of_smote, best_AUC_of_obj_func = run_experiment('changes', split_data)
    print('The best k_neighbors of changes:', k_of_smote)
    eval_result('changes', split_data)
