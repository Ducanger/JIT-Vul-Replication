...
def get_combined_features(code_commit, commit_id, label, metrics_df, count_vect, mode='train'):
    ...
    if mode == 'train':
        ...
        #     col_names = list(count_vect.get_feature_names_out())
        # else:
        final_features = np.concatenate((code_change_arr, metrics_df_arr), axis=1)
        col_names = list(count_vect.get_feature_names_out()) + list(metrics_df.columns)
        return final_features, col_names, list(code_df['label'])
    elif mode == 'test':
        code_features = pd.DataFrame(code_change_arr, columns=count_vect.get_feature_names_out())
...
def eval_with_LIME(proj_name, clf, explainer, test_features):
    ...
    for commit in correctly_predicted_commit:
        idx += 1
        if idx % 10 == 0: print(f'Process: {idx}/{len(correctly_predicted_commit)} ...')
        code_change_from_line_level_df = list(
            line_level_df[line_level_df['commit_id'] == commit]['changed_line'])
...
def eval_line_level(proj_name, best_k_neighbor, split_data='by_time'):
    # load model here
...
def eval_line_level_at_commit(cur_proj, data_path):
    ...
    for commit in all_commits:
        idx += 1
        if idx % 100 == 0: print(f'Process: {idx}/{len(all_commits)} ...')
        IFA_list = []
...
if __name__ == '__main__':
    ...
    print("Start eval_line_level_at_commit")
